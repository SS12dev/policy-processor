...
try:
    ...
    from .agent_executor import ApplicationProcessingAgentExecutor
...
def create_agent_card(host: str, port: int) -> AgentCard:
    ...
    process_application_skill = AgentSkill(
        id='process_application',
        name='Application Processing',
        description='Processes medical applications to extract patient summaries and answer questionnaire questions using multimodal analysis',
        tags=['medical', 'application', 'processing', 'multimodal', 'questionnaire'],
        examples=[
            'Process patient application and answer questionnaire',
            'Extract patient summary and respond to questions',
            'Analyze medical forms and provide structured responses',
            'Complete application processing with multimodal analysis'
        ],
    )
    ...
    agent_card = AgentCard(
        ...
        default_input_modes=ApplicationProcessingAgent.SUPPORTED_CONTENT_TYPES,
        default_output_modes=ApplicationProcessingAgent.SUPPORTED_CONTENT_TYPES,
        capabilities=capabilities,
        skills=[process_application_skill],
    )
...
def create_app():
    ...
    try:
        ...
        request_handler = DefaultRequestHandler(
            agent_executor=ApplicationProcessingAgentExecutor(),
        ...
        async def readiness_check(request):
            ...
            try:
                ...
                _ = ApplicationProcessingAgentExecutor()
                return JSONResponse({
                    "service": agent_card.name,
                    "probe": "readiness",
                    "status": "ready",
...
def main(host: str, port: int, log_level: str):
    """Start Application Processing Agent server with A2A protocol support."""
    try:
        # Update logging level
        setup_logging(log_level)
        logger.info("Application Processing Agent starting up...")
        ...
        logger.info(f"  Model: {agent_settings.AP_LLM_MODEL}")
        logger.info(f"  Endpoint: {agent_settings.AP_LLM_ENDPOINT}")
